...
c        p_23.f,sfm_23.f,coales_23.f,stahad_23.f,hadcas_23.f,and analy.f
...
c       stahad_23.f: hadronization with statistical model
...
c       16: # of allowable generations in q (qar) deexcitation in coales_23.f
...
c       29: For sfm in PYTHIA, it is MSTJ(11). Choice of longitudinal            !Lei20230214B
...
c           For coal, sampling daughter parton energy fraction z taking from     !
c            mother in 'funcz' for coal.                                         !
c           =1: by Lund string fragmentation function                            !
c           =2: by Field-Feynmman fragmentation function                         !
c           =3: null                                                             !
c           =4: null                                                             !
c           =5: null                                                             !Lei20230214E
...
c       nosc = 0 : no OSCAR output, see subroutine oscar                 !Lei20230214B
c            = 1 : OSCAR1997A (final_id_p_x, just PACIAE final output)   !
c            = 2 : OSCAR1999A (full_event_history)                       !
c            = 3 : OSCAR2013A (full_event_history)                       !Lei20230214E
c020708
...
c       tdh and itnum: not used now
c       cptl,cptu (cptl2,cptu2) : not used now
...
c       parp22: select y or eta in partial phase-space statistics.   !Lei20230214
c               = 0 , y
c               = 1 , eta
...
c
!Lei20230214 Choses fragmentation function in PYTHIA for sfm and 'funcz' for coal.
        MSTJ(11)=adj1(29)   !Lei20230214 D=4 in PYTHIA
...
!Lei20230214B
! Mistake-proofing.
! Values given automatically avoiding some manual input mistakes in usu.dat.
        if( ipden.eq.2 .AND. itden.eq.2 )then
! For e+e- , one just needs to specify ipden=itden=2 in usu.dat.
            nap =  1   ! nominal value
            nzp =  1   ! nominal value
            nat =  1   ! nominal value
            nzt = -1   ! nominal value
...
! For lN and lA, one just needs to specify ipden >= 11 and nat/nzt.
            nap = 1   ! nominal value
            nzp = 1   ! nomanal value
...
! For NN , NA(AN) and AA, one just needs to specify nap/nzp/nat/nzt. 
            ipden=0
            itden=0
            if(nap.gt.1) ipden=1
            if(nat.gt.1) itden=1
        ...
        if(win.lt.parp21) mstptj=1   ! if win.lt.parp21 & mstptj=0, deap loop in low-energy AA collision.
!Lei20230214E
...
c       gives values to some important variables
        ...
        if(nosc.gt.0) open(34,file='oscar.out',status='unknown')   !Lei20230214 nosc=0 : no oscar output
        call oscar(win,-1)   !Lei20230214 Prints file header if nosc > 0.
...
c       forbiden decay of particle (sets mdcy(...)=0)
        mdcy(pycomp(111),1)=1   ! pi0 260123
...
c       for nucleon (antinucleon)-nucleon (antinucleon) or e+e-
        ...
        tau=0.
c161222
        if((ipden.eq.0.and.itden.eq.0).and.win.lt.parp21)then  !!! 161222
...
c       (if ifram=1, b(i)=0 & \garma=1)
        endif

        ww=rcsit
        if(iii.eq.1)write(9,*)'inela./tot=rcsit=',rcsit
...
c       perform nucleon-nucleon (NN) elastic scattering
        call coelas(1,2,win,pi,pj)   ! in parini_23.f
c080123
        do i2=1,4
        psa(1,i2)=pi(i2)   ! four momentum of one collided particle
        psa(2,i2)=pj(i2)   ! four momentum of another one collided particle
        enddo
c080123
...
c       give four momentum to inelastically scattered particles 
        call coinel_nn(1,2,2214,2212,pi,pj)
c       decay of unstable hadron
c        write(22,*)'in main be. nsa,k=',nsa,ksa(1,2),ksa(2,2)
c        write(22,*)'p1=',psa(1,1),psa(1,2),psa(1,3)
c        write(22,*)'p2=',psa(2,1),psa(2,2),psa(2,3)
c        write(22,*)'e=',psa(1,4),psa(2,4)
        call padecy(1,2,0D0)
c        write(22,*)'af. main nsa,k=',nsa,ksa(1,2),ksa(2,2),ksa(nsa,2)
c        write(22,*)'p1=',psa(1,1),psa(1,2),psa(1,3)
c        write(22,*)'p2=',psa(2,1),psa(2,2),psa(2,3)
c        write(22,*)'pnsa=',psa(nsa,1),psa(nsa,2),psa(nsa,3)
c        write(22,*)'e=',psa(1,4),psa(2,4),psa(nsa,4)
        else
        ksa(1,2)=2224
        ksa(2,2)=2112
        ...
        elseif(nzp.eq.1 .and. nzt .eq.0)then   ! pn   !!
        if(rpy .gt. 0.5)then
        ksa(1,2)=2214
        ksa(2,2)=2112
        ...
        elseif(nzp.eq.0 .and. nzt .eq.1)then   ! np   !!
        if(rpy .gt. 0.5)then
        ksa(1,2)=2214
        ksa(2,2)=2112
        call coinel_nn(1,2,2214,2112,pi,pj)
        ...
        if(ifram.eq.0)then
c       boost back to CMS frame of colliding pair
        ilo=1
        call lorntz(ilo,b,pi,pj)
        do i2=1,4
        p(1,i2)=pi(i2)   ! four momentum of one collided particle
        p(2,i2)=pj(i2)   ! four momentum of another one collided particle
        enddo
        endif

        goto 998   ! toward hadron rescattering (call 'hadcas')

c       a nucleon-nucleon collision finished 
...
c       if(ipden.eq.2 .and. itden.eq.2)print*,'af. break'
        ...
        if(nosc.eq.2 .OR. nosc.eq.3) call oscar(win,1)   !Lei20230214 full_events_history of OSC1999A
...
c271205
        ...
        enddo
c020718
        if(adj140.eq.3)call coales(iii,neve,nout,nap,nat,nzp,nzt)
...
c201203
        ...
        if(nosc.eq.2 .OR. nosc.eq.3) call oscar(win,2)   !Lei20230214 full_events_history of OSC1999A
...
c230722
        ...
        if(nosc.eq.2 .OR. nosc.eq.3) call oscar(win,3)   !Lei20230214 full_events_history of OSC1999A
...
c       if(win.lt.parp21)then   ! 260123
        call decayh(rrp)   ! in sfm_23.f, 130205 260123
c       endif   ! 260123
...
c       'sbh' to 'pyjets' for hadronizing rest partons by coalescence
c220822
        n=nbh
        if(n.ge.1)then
        do i1=1,n
...
c100119 seting four components equal to 0 in order to have netba=0
...
c       ??????????????? OSCAR standard output ??????????????????????
        call oscar(win,4)   !Lei20230214 Outputs if nosc > 0
c       ??????????????? OSCAR standard output ??????????????????????
...
c       analyses the event on-line
        call analy(nmin,nminf,ncha,nchaf,parp22)   ! 281219   Lei20230214
c       sum over events        
        dnmin=dnmin+nmin
        dnminf=dnminf+nminf
        dncha=dncha+ncha
...
c071103
     ...
     c   'in parton cascade=',rineli,reli,reli+rineli
c071103
c220110
...
c220110
     ...
     c   ' in hadron cascade (at the end of the file)'   !Lei20230214 move to the end of rms.out.
!Lei20230214         write(10,*)dineli
...
!Lei20230214B
        write(10,*)   !Lei20230214 Empty line
        write(10,*)'average frequency of the occurring of each inela.'//
     c   ' in hadron cascade='
        do i=1,60,1
            j=(i-1)*6
...
c060813 statistics of processes generated
        ...
        close(9)
        close(22)
        if(nosc.gt.0) close(34)   !Lei20230214
c       close(98)   ! 260219
c       close(99)
...
!Lei20230214 The subroutine has been re-wrote.
...
c               = 4 : final program output, after PACIAE running.
...
c            = 3 : OSCAR2013A (full_event_history, dummy now)
        ...
        if(nosc.eq.0) return   ! No OSCAR output.
...
cccccccccccccccccccccccccccccc   OSC1999A   cccccccccccccccccccccccccccccc
            ...
            write(34,201) iii, neve, N, bp, phi, i_stage
