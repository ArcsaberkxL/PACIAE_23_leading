...
c        coales_23.f,hadcas_23.f,p_23.f,eps09.f, and analy.f
...
c       eps09.f: calculates eps09 nPDF
...
c        letter. typing ": set ic + enter key" before searching.
	...
	common/pycidat2/kfmaxt,nont2,param(20),weigh(600)
      COMMON/PYDATR/MRPY(6),RRPY(100)   !Lei20221017 For random number generator
...
!Lei20221017 MRPY(1) is the seed of PYTHIA random number generator.(D=19780503)
!Lei20221017 For the intrinsic subroutine DATE_AND_TIME calling.
      dimension n_current_date_and_time(8)   !Lei20221017
...
c       fraction for charge separation    ! she and Lei
        schun=0.
        schudn=0.
        schudsn=0.
        sfra=0.
...
!Lei20221017 Note it is an array. One doesn't need to initialize it using do-loop.
      n_current_date_and_time = 0   !Lei20221017
      i_seed = MRPY(1)   !Lei20221017

	if(iran.eq.0)goto 300
!Lei20221017	do i1=1,iran
!Lei20221017	thrr=pyr(1)
!Lei20221017	enddo

!Lei20221017B--
! A new method is introduced now.
! The initial seed of the random number generator will be given by real-time 
!   clock from computer.
! Get the current real-date_and_time from the computer.
...
! The seed will be 8-digit HH-MM-SS-ms-ms.
        MRPY(1) = i_seed
!Lei20221017E--
...
!Lei20221017 Records the seed of random number generator.
      write(10,*)"seed (PYTHIA default=19780503) = ", MRPY(1)   !Lei20221017
...
C        F((U+D)/2+(U-D)/2*X).
C     X IS NODE.
!Lei20221017      DO 101 I=1,N1 
      DO I=1,N1 
 101  X1(I)=0.5*(U1+D1+(U1-D1)*X11(I))
      END DO
!Lei20221017      DO 102 I=1,N2 
      DO I=1,N2 
 102  X2(I)=0.5*(U2+D2+(U2-D2)*X22(I))
      END DO
!Lei20221017      DO 103 I=1,N3 
      DO I=1,N3 
...
203   ROUP=3./4./3.1416*IAP/(RP*RP*RP)
	...
	IF(II.EQ.2)ROU=ROUT
      SUM(II)=0.
!Lei20221017      DO 105 I1=1,N1
      DO I1=1,N1
      XX1=X1(I1)
      SUM2=0.
!Lei20221017      DO 106 I2=1,N2
      DO I2=1,N2
      XX2=X2(I2)
      SUM3=0.
!Lei20221017      DO 107 I3=1,N3
      DO I3=1,N3
      XX3=X3(I3)
 107  SUM3=SUM3+W3(I3)*
...
C                                                                               
  100 W(1) = 1.0E0                                                              
!Lei20221017      DO 105 I = 2, N                                               
      DO I = 2, N                                                               
  105    W(I) = 0.0E0                                                           
      END DO                                                                    
C                                                                               
      CALL IMTQL2 (N, T, B, W, IERR)                                            
!Lei20221017      DO 110 I = 1, N                                               
      DO I = 1, N                                                               
...
C                                                                               
      ...
      C=1.0E0                                                                   
      Y=X                                                                       
!Lei20221017      IF(Y-2.0E0)1,2,2                                              
      IF((Y-2.0E0).LT.0E0) GOTO 1                                               
!Lei20221017 2    IF(Y-3.0E0)3,3,4                                              
 2    IF((Y-3.0E0).LE.0E0) GOTO 3                                               
!Lei20221017 4    IF(Y-10.0E0)5,5,6                                             
 4    IF((Y-10.0E0).GT.0E0) GOTO 6                                              
 5    Y=Y-1.0E0                                                                 
      C=C*Y                                                                     
!Lei20221017      IF(Y-3.0E0)3,3,5                                              
